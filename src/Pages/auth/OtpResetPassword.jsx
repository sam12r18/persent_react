...
export default function OtpResetPassword() {
    ...
    const {mobile , setPasswordHash} = useAuth()
    const {
        control,
        handleSubmit,
        formState: { errors },
    ...
    const onSubmit = async (data) => {
        ...
        try {
             const response = await apiPost(`auth/check-otp/change-password`, { code: otpCode, mobile: mobile });
            setPasswordHash(response?.hash);
            console.log("this is response for reset password");
            setTimeout(() => {
    ...
    return (
        <Container className="container-sm mt-3" dir="rtl">
            <div className="d-flex flex-column">
                <PageTitle title={"تایید کد"}/>
                <div className="d-flex flex-column gap-2 p-2">
                    <span className="fs-4 fw-bold mb-3">کد تایید تغییر رمز عبور</span>
