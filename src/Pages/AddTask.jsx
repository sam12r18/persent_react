...
export default function AddTask() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      await Toast.fire({
        icon: "success",
        title: "وظیفه شما با موفقیت ثبت شد",
      });
    ...
    <Container className="container-sm align-items-center justify-content-center mt-3">
      <div className="row">
        ...
        <div className="mt-5 d-flex flex-column">
          <span className="fs-4 fw-bold mb-3">ایجاد وظیفه برای کارمند:</span>
          <span className="text-color mb-3 fs-5">عرفان محسنی</span>
        </div>
        <div className="mt-5 mb-5">
          <Form onSubmit={handleSubmit(onSubmit)}>
            <Form.Group
              className="mb-5 mt-6 position-relative"
              controlId="taskTitle"
            >
              <Form.Label
                column="sm"
                className={`rounded-4 fs-6 py-2 px-2 ${
                  isFocused || errors.task_title ? "text-orange" : "text-color"
                }`}
              >
                نام وظیفه
              </Form.Label>
              <Form.Control
                ...
                className="rounded-5 py-3 no-arrows text-right"
                {...register("task_title", {
            ...
            <Form.Group
              className="mb-5 position-relative mt-5"
              controlId="taskDescription"
            >
              <Form.Label
                column="sm"
                className={`rounded-4 fs-6 py-2 px-2 ${
                  isFocused2 || errors.task_description
                    ? "text-orange"
                    : "text-color"
                }`}
              >
